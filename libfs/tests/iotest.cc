...
void io_bench::prepare(void)
{
  ...
  if (test_type == SEQ_READ || test_type == RAND_READ) {
    ...
    if ((fd = open(test_file.c_str(), flags, 0666)) < 0)
      err(1, "open");
  } else {
...
{
  cout << "thread " << id << " start - file: " << test_file << endl;

  if (test_type == SEQ_READ || test_type == RAND_READ || test_type ==
      ZIPF_READ) {
    this->do_read();
  } else {
    this->do_write();
...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < n_threads; i++) {
    io_workers.push_back(new io_bench(i, file_size_bytes, io_size,
